...
// I386 (IMAGE_REL_I386_*)
// This package only encodes the subset that has an equivalent on AMD64/ARM64
// and thus a corresponding coff.RelocKind: DIR32, DIR32NB, REL32, SECREL.
// SEG12 (0x0009), TOKEN (0x000C, CLR-only), and SECREL7 (0x000D) have no
// RelocKind and are not reachable through this package's API.
const (
	relI386Dir32   uint16 = 0x0006 // 32-bit VA
	relI386Dir32NB uint16 = 0x0007 // 32-bit image-relative (IAT)
	relI386Secrel  uint16 = 0x000B // 32-bit section-relative (TLS IE)
	relI386Rel32   uint16 = 0x0014 // 32-bit PC-relative
)
...
func (f *File) relocType(k RelocKind) (uint16, error) {
	...
	case machineI386:
		switch k {
		case RelocAbs32:
			return relI386Dir32, nil
		case RelocPCRel32, RelocPLT32:
			return relI386Rel32, nil
		case RelocIAT:
			return relI386Dir32NB, nil
		case RelocAddr32NB:
			return relI386Dir32NB, nil
		case RelocTLSIE:
			return relI386Secrel, nil
		}
	case machineAMD64:
		switch k {
		case RelocAbs64:
...
// Only 64-bit absolute relocations use an 8-byte field; everything else uses
// 4. I386 has no 64-bit relocation type at all (RelocAbs64 is rejected by
// relocType before this is ever consulted for machineI386), so it always
// falls into the 4-byte case here.
...
// default when s.Align is zero. This is machine-independent — i386 uses the
// same defaults as AMD64/ARM64.
...
//
// This does not add the leading underscore i386 cdecl/stdcall decoration
// expects, nor stdcall's trailing "@N" — callers targeting TargetWindowsX86
// must bake that into Symbol.Name/Reloc.Symbol themselves before it reaches
// this function; see the i386 caveat in coff.go's package doc.
...
// for an AMD64 REL32 or an I386 REL32 site must be A + 4. Without this the -4
// is applied twice (once here, once by REL32's definition) and every
// call/RIP (or, on i386, call/jmp) reference lands 4 bytes short.
//
// The bump is gated on machine+type pairs (relAMD64Rel32, relI386Rel32)
// rather than on the raw type value alone, because relAMD64Rel32 (0x0004),
// relARM64Branch26 (0x0003), and relI386Rel32 (0x0014) all live in
// overlapping-looking numeric ranges across machines coincidentally, not by
// shared meaning; only the two true REL32 cases need the +4 — ARM64's
// BRANCH26 path uses addend 0 with an S+A-P patcher and must not be
// adjusted.
//
...
func applyImplicitAddends(code []byte, relocs []Reloc, rtypes []uint16, machine uint16) []byte {
	...
	for i, r := range relocs {
		...
		addend := r.Addend
		isRel32 := (machine == machineAMD64 && rtypes[i] == relAMD64Rel32) ||
			(machine == machineI386 && rtypes[i] == relI386Rel32)
		if isRel32 {
			addend += 4
		}
